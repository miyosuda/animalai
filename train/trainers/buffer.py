...
class Buffer(dict):
    ...
    Buffer also contains an update_buffer that corresponds to the buffer used when updating
    the model.
    """

    class AgentBuffer(dict):
        """
        AgentBuffer contains a dictionary of AgentBufferFields. Each agent has his 
        own AgentBuffer.
        ...
        class AgentBufferField(list):
            """
            AgentBufferField is a list of numpy arrays. When an agent collects a field, 
            you can add it to his
            ...
            def get_batch(self,
                ...
                if training_length == 1:
                    ...
                    if batch_size is None:
                        # If batch_size is None : All the elements of the AgentBufferField
                        # are returned.
                    ...
                    # The training_length is not None, the method returns a list of SEQUENCES
                    # of elements
                    if not sequential:
                        ...
                        # The number of sequences of length training_length taken from a
                        # list of len(self) elements
                        # with overlapping is equal to batch_size
                        if (len(self) - training_length + 1) < batch_size:
                            raise BufferException(
                                "The batch size and training length requested for get_batch"
                                " where too large given the current number of data points.")
                        tmp_list = []
                        for end in range(len(self) - batch_size + 1, len(self) + 1):
                            tmp_list += [np.array(self[end - training_length:end])]
                        return np.array(tmp_list)
                    if sequential:
                        # The sequences will not have overlapping elements
                        # (this involves padding)
                        leftover = len(self) % training_length
                        # leftover is the number of elements in the first sequence
                        # (this sequence might need 0 padding)
                        if batch_size is None:
                            # retrieve the maximum number of elements
                            batch_size = len(self) // training_length + 1 * (leftover != 0)
                        # The maximum number of sequences taken from a list of length
                        # len(self) without overlapping
                        ...
                        # The padding is made with zeros and its shape is given by the
                        # shape of the last element
                        for end in range(len(self),
                                         len(self) % training_length,
        ...
        def check_length(self, key_list):
            ...
            :key_list: The fields which length will be compared
            """
            if len(key_list) < 2:
                return True
            l = None
        ...
        def shuffle(self, key_list=None):
            ...
            :key_list: The fields that must be shuffled.
            """
            if key_list is None:
        ...
        def make_mini_batch(self, start, end):
            ...
            :start: Starting index of buffer.
            :end:   Ending index of buffer.
    ...
    def append_update_buffer(self,
        ...
        :agent_id:        The id of the agent which data will be appended
        :key_list:        The fields that must be added. If None: all fields will be appended.
        :batch_size:      The number of elements that must be appended. 
                          If None: All of them will be.
        :training_length: The length of the samples that must be appended. 
                          If None: only takes one element.
        """
        if key_list is None:
        ...
        for field_key in key_list:
            self.update_buffer[field_key].extend(
                self[agent_id][field_key].get_batch(batch_size=batch_size,
                                                    training_length=training_length))

    def append_all_agent_batch_to_update_buffer(self,
                                                key_list=None,
                                                batch_size=None,
                                                training_length=None):
        """
