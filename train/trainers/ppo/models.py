...
class PPOModel(LearningModel):
    def __init__(self,
        ...
        :param brain: 
            BrainInfo used to generate specific network graph.
        :param lr: 
            Learning rate.
        :param h_size: 
        ...
        :param normalize: 
            Whether to normalize vector observation input.
        :param use_recurrent: 
            Whether to use an LSTM layer in the network.
        :param num_layers:
            Number of hidden layers between encoded input and policy & value layers
        :param m_size: 
            Size of brain memory.
        ...
        self.create_ppo_optimizer(self.log_probs,
                                  self.old_log_probs,
                                  self.value,
                                  self.entropy,
                                  beta,
    ...
    def create_curiosity_encoders(self):
        ...
        :return: 
        current and future state encoder tensors.
        """
        encoded_state_list = []
        ...
        if self.vis_obs_size > 0:
            ...
            for i in range(self.vis_obs_size):
                ...
                # Not that these encoders are siamese.
        ...
        if self.vec_obs_size > 0:
            ...
            self.next_vector_in = tf.placeholder(shape=[None, self.vec_obs_size],
                                                 dtype=tf.float32,
    ...
    def create_inverse_model(self, encoded_state, encoded_next_state):
        ...
        :param 
            encoded_state: Tensor corresponding to encoded current state.
        :param 
            encoded_next_state: Tensor corresponding to encoded next state.
        """
    ...
    def create_forward_model(self, encoded_state, encoded_next_state):
        ...
        hidden = tf.layers.dense(combined_input, 256, activation=self.swish)
        # We compare against the concatenation of all observation streams,
        # hence `self.vis_obs_size + int(self.vec_obs_size > 0)`.
        ...
        squared_difference = 0.5 * tf.reduce_sum(tf.squared_difference(
            pred_next_state, encoded_next_state), axis=1)
    ...
    def create_ppo_optimizer(self,
        ...
        self.value_loss = tf.reduce_mean(tf.dynamic_partition(
            tf.maximum(v_opt_a, v_opt_b), self.mask, 2)[1])

        # Here we calculate PPO policy loss. In continuous control this is
        # done independently for each action gaussian
        # and then averaged together. This provides significantly better performance
        # than treating the probability
